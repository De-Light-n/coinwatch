...
class WatchService:
    ...
    async def create_watch(self, data: WatcherCreate, user_id: str) -> Watch:
        ...
        created_watch = await self.repo.create(watch)
 
        try:
            from app.tasks import check_watch
            check_watch.apply_async(
                args=[str(created_watch.id)],
                queue='watcher_high'
            )
            logger.info(f"Eager check dispatched for watch {created_watch.id}")
        except Exception as e:
            logger.error(f"Eager check dispatch failed for {data.asset}: {e}")
    ...
    async def search_coins(self, query: str):
        return await self.coingecko_client.search_coins(query)
