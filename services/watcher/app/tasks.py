...
from app.coingecko.exceptions import CoinGeckoUnavailableError, CoinGeckoInvalidCoinError
        ...
        async with async_session_factory() as session:
            ...
            try:
                ...
                if watch["condition_type"] == "price_above":
                    if current_price > watch["threshold"]: triggered = True
                    ...
                    if prev_price_str:
                        ...
                        if prev_price > 0:
                            ...
                            if change > watch["threshold"]: triggered = True
                    else:
                    ...
                    if market_rank <= watch["threshold"]: triggered = True
                ...
                if triggered:
                    ...
                    if watch["last_triggered_at"]:
                        ...
                        if now - watch["last_triggered_at"] < cooldown:
                            can_trigger = False
                    if can_trigger:
